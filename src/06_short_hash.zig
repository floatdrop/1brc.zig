...
const StatsMap = std.HashMap([]const u8, Stats, ShortStringContext, std.hash_map.default_max_load_percentage);

const ShortStringContext = struct {
    pub fn hash(self: @This(), s: []const u8) u64 {
        _ = self;
        // Make sure we allways use smallKey opeimization
        return std.hash.Wyhash.hash(0, s[0..@min(s.len, 16)]);
    }
